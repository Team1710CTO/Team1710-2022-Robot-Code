...
public class IntakeSubsystem extends SubsystemBase {
  ...
  public IntakeSubsystem() {
    m_actuatorLeft =
      new CANSparkMax(
        Constants.LEFT_INTAKE_ACTUATOR_CAN_ID,
    ...
    m_actuatorLeft_PidController.setOutputRange(
      Constants.INTAKE_LEFT_kMinOutput,
      Constants.INTAKE_LEFT_kMaxOutput
    );
    m_actuatorLeft_PidController.setD(Constants.INTAKE_LEFT_kD);

    m_actuatorRight =
      new CANSparkMax(
        Constants.RIGHT_INTAKE_ACTUATOR_CAN_ID,
        MotorType.kBrushless
      );
    ...
    m_actuatorRight_PidController.setOutputRange(
      Constants.INTAKE_RIGHT_kMinOutput,
      Constants.INTAKE_RIGHT_kMaxOutput
    );
  ...
  public void periodic() {
    ...
    SmartDashboard.putNumber(
      "intake rotations position",
      m_actuatorLeft_encoder.getPosition()
    );

    SmartDashboard.putNumber(
      "intake rotations velo",
      m_actuatorLeft_encoder.getVelocity()
    );

    SmartDashboard.putBoolean("is current bol treu", isIntakeStalledCurrent());

    SmartDashboard.putBoolean(
      "is velociuty basically zero",
      isIntakeVelocityBasicallyZero()
    );
    SmartDashboard.putString("intake STAtes", intakeState);

    if (
      Math.abs(
        m_actuatorRight_encoder.getPosition() - Constants.INTAKE_RIGHT_up
      ) <
      1
    ) {
      intakeState = "Up";
      ...
      Math.abs(
        m_actuatorRight_encoder.getPosition() - Constants.INTAKE_RIGHT_down
      ) <
      1
    ) {
      intakeState = "Down";
  ...
  private static void stopIntakeRunner() {
    m_intakeRunner.set(
      ...
      Constants.INTAKE_RUNNER_SPEED_OFF
    );
  }
  ...
  public static void runIntakeRunner() {
    m_intakeRunner.set(
      ControlMode.PercentOutput,
      Constants.INTAKE_RUNNER_SPEED_ON
    );
  }

  public void setIntakeUp() {
    if (isZeroed) {
      m_actuatorLeft_PidController.setReference(
        Constants.Intake_LEFT_up,
        CANSparkMax.ControlType.kPosition
      );
      m_actuatorRight_PidController.setReference(
        Constants.INTAKE_RIGHT_up,
        CANSparkMax.ControlType.kPosition
      );
    } else {
      SmartDashboard.putString("Intake Status", "!!Not Zeroed!!");
  ...
  public void setintakeDown() {
    if (isZeroed) {
      m_actuatorLeft_PidController.setReference(
        Constants.INTAKE_LEFT_down,
        CANSparkMax.ControlType.kPosition
      );
      m_actuatorRight_PidController.setReference(
        Constants.INTAKE_RIGHT_down,
        CANSparkMax.ControlType.kPosition
      );
    } else {
      SmartDashboard.putString("Intake Status", "!!Not Zeroed!!");
  ...
  public static double getIntakeActuatorAverageVelocity() {
    return (
      (getIntakeLeftActuatorVelocity() + getIntakeRightActuatorVelocity()) / 2
    );
  }
  ...
  public static boolean isIntakeVelocityBasicallyZero() {
    return (
      Math.abs(getIntakeLeftActuatorVelocity()) <
      Constants.INTAKE_ZERO_VELOCITY_THRESHOLD_UB
    );
  }

  public boolean isIntakeStalledCurrent() {
    return getIntakeActuatorCurrent() > Constants.INTAKE_CURRENT_LIMIT;
  }

  public static double getIntakeActuatorCurrent() {
    return (
      m_actuatorLeft.getOutputCurrent() + m_actuatorRight.getOutputCurrent()
    );
  }
