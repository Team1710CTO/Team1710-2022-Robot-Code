...
public class GyroSubsystem extends SubsystemBase {
  ...
  private static final PigeonIMU m_rightPigeon = new PigeonIMU(
    Constants.RIGHT_PIGEON_ID
  );
  //private final static PigeonIMU m_leftPigeon = new PigeonIMU(Constants.LEFT_PIGEON_ID);
  private static final AHRS m_navx = new AHRS();
  ...
  public static Rotation2d getNavXRotationAuto() {
    if (m_navx.isMagnetometerCalibrated()) {
      // We will only get valid fused headings if the magnetometer is calibrated
      return Rotation2d.fromDegrees(m_navx.getFusedHeading());
    }
