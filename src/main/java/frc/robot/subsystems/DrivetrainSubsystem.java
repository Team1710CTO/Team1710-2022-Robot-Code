...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  // TODO: Measure this?
  public static final double MAX_ACCEL_METERS_PER_SECOND_SQUARED = 10;
  ...
  private static ChassisSpeeds m_chassisSpeeds = new ChassisSpeeds(
    0.0,
    0.0,
    0.0
  );
  ...
  /**
   * see gyro subsystem for all gyro stuff
   *
   */
  ...
  public void periodic() {
    m_chassisSpeeds.omegaRadiansPerSecond =
      headingControlModifier(headingControlEnanbled());
    ...
    ChassisSpeeds actualChassisSpeeds = m_kinematics.toChassisSpeeds(
      actualStates
    );
    ...
    SmartDashboard.putNumber(
      "robot omega velocity",
      actualChassisSpeeds.omegaRadiansPerSecond
    );
  ...
  public static double headingControlModifier(boolean active) {
    if (active) {
      ...
      if (
        Math.abs(m_chassisSpeeds.omegaRadiansPerSecond) >
        Constants.ROTATION_PID_SUPPLIER_ACTIVATION_THRESHOLD ||
        ...
        if (
          pidActivationIterator >
          Constants.ROTATION_PID_ITERATOR_ACTIVATION_THRESHOLD
        ) {
          ...
          SmartDashboard.putNumber(
            "rot Error",
            rotationPidController.getPositionError()
          );
  ...
  public void DriveToPosition(Pose2d desiredPose2d) {
    drive(
      ChassisSpeeds.fromFieldRelativeSpeeds(
        ...
        rotationPidController.calculate(
          GyroSubsystem.getBestRotation2d().getDegrees(),
          desiredPose2d.getRotation().getDegrees()
        ),
        GyroSubsystem.getBestRotation2d()
      )
    );
  ...
  public boolean isInPosition(Pose2d desiredPose2d) {
    if (
      (Math.abs(m_odometry.getPoseMeters().getX() - desiredPose2d.getX())) <
      .1 &&
      (Math.abs(m_odometry.getPoseMeters().getY() - desiredPose2d.getY())) < .1
    ) {
      return true;
    } else {
      return false;
