...
public class IntakeWithVision extends CommandBase {
  ...
  public void execute() {
    ...
    SmartDashboard.putNumber(
      "poopoo",
      (xPidController.getPositionError() + yPidController.getPositionError())
    );

    // If ball is close, lower pickup and run intake.
    if (
      photonVisionSubsystem.hasBallTargets() &&
      Math.abs(xPidController.getPositionError()) < 15 &&
      Math.abs(yPidController.getPositionError()) < 4 &&
      intakeSubsystem.getIntakeState() == "Up"
    ) {
      intakeSubsystem.setintakeDown();
      intakeSubsystem.runIntake();
    }

    // If intake is up, lets go ahead and path to ball.
  ...
  public boolean isFinished() {
    // Once we have gotten the ball under our control, we're done.
    return (
      !(initialballcount == indexerSubsystem.getBallCount()) ||
      !indexerSubsystem.topBeamBreak.get()
    );
  }
}
